...
public static class MeanStrategyExecutor
{
    ...
    private static List<StockData> LoadData(string path, bool cache = false)
    {
        if (!cache)
        {
            return StockDataLoader.Load(path).ToList();
        }
        ...
        {
            DataCaches.Clear();
            DataCaches[date] = StockDataLoader.Load(path).ToList();
        }

        return DataCaches[date].ToList();
    }
    ...
    )
    {
        ...
        var result = list.ToList()
            ...
            .Where(
                o =>
                    o.HistoryData[^1].ClosingPrice
                    <= o.HistoryData.Select(e => e.ClosingPrice).Min()
                        * (1 + parameters.Percentage / 100.0)
            )
